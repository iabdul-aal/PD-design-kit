...
cfg = struct('lambda_center', 1310e-9, 'lambda_min', 1260e-9, 'lambda_max', 1360e-9, ...
    'eta_plateau', 0.95, 'Id', 1.3e-9, 'Psat', 10e-3, 'f3dB', 103e9, 'Rs', 12.9, 'Cj', 22.6e-15, ...
    'Lp', 175.3e-12, 'Cp', 10e-15, 'symbol_rate', 53.125e9, 'samples_per_symbol', 8, ...
    'num_bits', 100000, 'P_avg_dBm', -2, 'extinction_ratio', 10, ...
    'R_load', 50, 'T', 300, 'channel_noise_variance', 1e-8, 'enable_shot_noise', true, ...
    'enable_thermal_noise', true, 'plot_num_symbols', 100, 'figure_dir', figureDir, ...
    'h', 6.626e-34, 'c', 3e8, 'e', 1.602e-19, 'k', 1.381e-23, 'font_size', 11, ...
    'title_size', 13, 'label_size', 12, 'line_width', 2.5, 'grid_alpha', 0.15, ...
    'font_name', 'Times New Roman', 'export_dpi', 300);
cfg.colors = struct('blue', [0, 0.447, 0.741], 'red', [0.85, 0.325, 0.098], ...
    'green', [0.466, 0.674, 0.188], 'darkred', [0.635, 0.078, 0.184], 'gray', [0.5, 0.5, 0.5]);
end
...
xline(cfg.lambda_min * 1e9, '--', 'Color', cfg.colors.gray, 'LineWidth', 1.5); xline(cfg.lambda_max * 1e9, '--', 'Color', cfg.colors.gray, 'LineWidth', 1.5);
plot(cfg.lambda_center * 1e9, responsivity, 'o', 'Color', cfg.colors.red, 'MarkerFaceColor', cfg.colors.red, 'LineWidth', 1.5);
xlabel('Wavelength, lambda (nm)', 'FontSize', cfg.label_size, 'FontWeight', 'bold'); ylabel('Responsivity, R (A/W)', 'FontSize', cfg.label_size, 'FontWeight', 'bold');
title(sprintf('Photodiode Responsivity over %.0f-%.0f nm', cfg.lambda_min * 1e9, cfg.lambda_max * 1e9), 'FontSize', cfg.title_size, 'FontWeight', 'bold');
grid on; xlim([300, 1800]); ylim([0, max(1, 1.1 * max(R))]); styleAxes(cfg); saveThesisFigure(fig, cfg.figure_dir, fileName, cfg.export_dpi);
end
...
xline(Pmin * 1e3, '--', 'Color', cfg.colors.green, 'LineWidth', 1.5); xline(Pmax * 1e3, '--', 'Color', cfg.colors.red, 'LineWidth', 1.5);
xline(cfg.Psat * 1e3, ':', 'Color', cfg.colors.darkred, 'LineWidth', 1.5); yline(Isat * 1e3, ':', 'Color', cfg.colors.darkred, 'LineWidth', 1.5);
xlabel('Optical Power, P (mW)', 'FontSize', cfg.label_size, 'FontWeight', 'bold'); ylabel('Photocurrent, I (mA)', 'FontSize', cfg.label_size, 'FontWeight', 'bold');
title(sprintf('Transfer Function at %d nm', round(cfg.lambda_center * 1e9)), 'FontSize', cfg.title_size, 'FontWeight', 'bold');
grid on; xlim([0, max(P) * 1e3]); ylim([0, 1.1 * max(Iactual) * 1e3]); styleAxes(cfg); saveThesisFigure(fig, cfg.figure_dir, fileName, cfg.export_dpi);
end
...
if isPower, tone = 0.75 * cfg.colors.blue + 0.25 * [1, 1, 1]; yLabel = 'Power, P (mW)'; else, yLabel = 'Current, I (mA)'; end
fig = figure('Color', 'w', 'Position', [120, 120, 1600, 1000], 'Name', [signalType, ' Eye Diagram']);
plot(tEye, eyeData, 'Color', tone, 'LineWidth', 0.6);
xlabel('Time, t (ps)', 'FontSize', cfg.label_size, 'FontWeight', 'bold'); ylabel(yLabel, 'FontSize', cfg.label_size, 'FontWeight', 'bold');
title(sprintf('%s Eye Diagram (SNR = %.1f dB)', signalType, snrDb), 'FontSize', cfg.title_size, 'FontWeight', 'bold');
grid on; xlim([tEye(1), tEye(end)]); styleAxes(cfg); saveThesisFigure(fig, cfg.figure_dir, fileName, cfg.export_dpi);
end
