...
\subsection{IEEE 802.3 400GBASE-DR4 as the Receiver-Level Target}
\label{subsec:pd_ieee_context}

The literature-based design choice also has to be justified against the target
system specification. IEEE~802.3 Clause~140 defines 400GBASE-DR4 as a
\SI{400}{\giga\bit\per\second} single-mode-fibre interface in the O-band,
implemented through four parallel optical lanes at \SI{1310}{\nano\metre} over
reaches up to \SI{500}{\metre}~\cite{IEEE80232022}. Each lane carries
\SI{106.25}{\giga\bit\per\second} raw line rate with PAM-4 signalling, so the
receiver-side photodetector must be judged not only by responsivity and dark
current, but also by the bandwidth, optical power range, and noise conditions
implied by the standard. The key receiver-side quantities are summarised in
\cref{tab:ieee_specs}.

\begin{table}[H]
\centering
\caption{IEEE~802.3 400GBASE-DR4 receiver-side constraints most directly
relevant to the photodetector, extracted from Clause~140~\cite{IEEE80232022}
and mapped onto the device metrics of \cref{sec:pd_concept}. The final column
identifies the principal design relation where an explicit detector-level
mapping is used in this chapter.}
...
This standard-based framing clarifies why the \SI{103}{\giga\hertz}-class
single-U benchmark is the appropriate literature anchor for the present work.
...
establishes comfortable speed relevance for DR4. The more demanding issue is
not whether the chosen architecture can operate fast enough, but whether it can
remain linear across the receive-power window imposed by the standard. The
maximum receive power of \SI{+3.4}{\dBm} (\SI{2.19}{\milli\watt}) is far above
the screening regime later examined through \cref{eq:p_screening}, whereas the
minimum OMA of \SI{-6.0}{\dBm} (\SI{251}{\micro\watt}) lies much closer to the
same operating range. Accordingly, IEEE~802.3 400GBASE-DR4 is used here not as
a late-stage compliance claim, but as the receiver-level constraint that
justifies the selected topology and anticipates the dynamic-range problem
resolved in the later device analysis.

From the standard perspective, the selected design is not the literature device
with the largest raw bandwidth, but the one that satisfies the DR4 receiver
requirements in the most balanced way. The lateral Ge-fin benchmark of
\SI{265}{\giga\hertz} in \cref{tab:pd_lit_benchmarks} exceeds the DR4 bandwidth
target by a very large margin, but it does so with markedly lower
responsivity and substantially higher dark current than the selected vertical
benchmark. Earlier vertical devices preserve stronger detector efficiency, yet
they leave much less bandwidth margin or retain higher leakage. By contrast,
the U-shaped vertical Ge-on-Si PIN of Shi~\textit{et al.}~\cite{Shi2024}
combines \SI{103}{\giga\hertz}-class response,
$\mathcal{R}=\SI{0.95}{\ampere\per\watt}$, and
$I_d=\SI{1.3}{\nano\ampere}$ in a process-compatible O-band platform. Because
DR4 requires a detector that is simultaneously fast enough, efficient at low
OMA, and quiet enough not to erode receiver margin, this balanced vertical
U-shaped architecture is the most defensible starting point for the present
design rather than the absolute-bandwidth record holder.
...
of the DR4 receive-power window introduced in \cref{subsec:pd_ieee_context},
