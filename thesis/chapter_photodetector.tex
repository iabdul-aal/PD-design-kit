...
This literature-based selection is also consistent with the intended
application. IEEE~802.3 400GBASE-DR4 specifies four O-band lanes at
\SI{106.25}{\giga\bit\per\second} using PAM-4 signalling, corresponding to a
symbol rate of \SI{53.125}{\giga\baud} per lane and a minimum receiver
bandwidth of \SI{39.8}{\giga\hertz} under the conventional $0.75f_s$
criterion~\cite{IEEE80232022}. The \SI{103}{\giga\hertz}-class single-U
benchmark therefore exceeds the required electrical bandwidth with substantial
margin. The standard is used here only as a detector-level relevance check for
the selected topology; the chapter itself remains organised around the device
physics and the subsequent O-band re-optimisation.
...
is illustrated in \cref{fig:design_dimensions}, its principal geometrical and
stack parameters are summarised in \cref{tab:design_specifications}, and the
corresponding mask-level layout is indicated in \cref{fig:klayout_layout}.
...
\begin{figure}[H]
    ...
    \missingfigure[figwidth=0.86\linewidth]{
        ...
        \SI{5}{\micro\metre} mesa width, and the U-shaped anode clearance.
    }
    \caption{Selected device geometry for the Ge-on-Si PIN photodetector,
    showing the principal dimensions used in the FDTD and CHARGE models.}
    \label{fig:design_dimensions}
\end{figure}

\begin{table}[H]
\centering
\caption{Geometrical dimensions and simulation-relevant specifications of the
selected Ge-on-Si PIN photodetector.}
\label{tab:design_specifications}
\footnotesize
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{>{\RaggedRight\arraybackslash}p{3.6cm} >{\RaggedRight\arraybackslash}p{2.4cm} >{\RaggedRight\arraybackslash}p{5.3cm}}
\toprule
Parameter & Value & Role in the model \\
\midrule
Operating wavelength & \SI{1310}{\nano\metre} & O-band design target used in the optical simulation and responsivity extraction \\
Ge thickness & \SI{350}{\nano\metre} & Intrinsic absorption and transit-time thickness \\
Ge width & \SI{5}{\micro\metre} & Mesa width governing junction area and lateral collection path \\
Ge length & \SI{8}{\micro\metre} & Absorption length selected from the O-band sweep \\
Intrinsic region width & \SI{3}{\micro\metre} & Depleted collection region in the vertical PIN stack \\
Taper length & \SI{40}{\micro\metre} & Adiabatic optical coupling from the Si waveguide to the Ge absorber \\
U-shaped contact width & \SI{1}{\micro\metre} & M1 anode wrap used to reduce lateral series resistance \\
$n^{++}$ contact width & \SI{0.5}{\micro\metre} & Cathode contact definition above the Ge mesa \\
Via pitch & \SI{0.5}{\micro\metre} & Electrical interconnect density between the metal levels \\
M1 thickness & \SI{0.5}{\micro\metre} & Local contact metal in the layout abstraction \\
M2 thickness & \SI{2}{\micro\metre} & Routing metal for the external electrical runner \\
BOX thickness & \SI{2}{\micro\metre} & Optical isolation from the Si substrate \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
