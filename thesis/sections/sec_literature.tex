...
The carrier transport physics, velocity saturation dynamics, bandwidth
trade-offs, dark current decomposition, and quantum efficiency formalism
that underpin the benchmarked designs in \cref{tab:pd_lit_benchmarks}
have been developed from first principles in
\cref{sec:pd_fundamentals,sec:pd_metrics}. The key quantitative
relationships---the transit-time bandwidth of \cref{eq:bw_tt}, the RC
bandwidth of \cref{eq:bw_rc}, the combined quadrature bandwidth of
\cref{eq:bw_combined}, and the SRH dark-current scaling of
\cref{eq:jdark_bulk}---are applied directly in the design and simulation
chapters that follow without re-derivation. Among the coupled design
variables ($W_i$, $L$, $A_j$, $R_s$, $L_p$), reducing $R_s$
through electrode engineering is the structurally cleanest intervention
because it improves $f_{\mathrm{RC}}$ without degrading responsivity or
dark current---an observation that provides the quantitative foundation
for the U-shaped anode topology adopted in \cref{sec:pd_design}.